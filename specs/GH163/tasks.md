...
- [x] `SP163-T2` 在共享 semantic validator 与 JSON gate 中双向校验 degraded status、非空白授权、Summary marker 与禁止性 claim。Covers: B-004 B-005 B-006。Owner: implementation lane。Depends on: T1。Done when: JSON gate 与 manifest 对所有缺失、越界、错位和矛盾组合都返回 blocked。Verify: `uvx pytest tests/test_review_json_gate.py -q -k 'gate_status or gate_authorization or degraded or marker or unavailable'`。
- [x] `SP163-T3` 扩展 review result schema、接入共享 semantic validator 并保持 legacy compatibility。Covers: B-007 B-008。Owner: implementation lane。Depends on: T2。Done when: 合规 degraded artifact 通过 manifest loader，孤立/空白授权与错位 marker 被拒绝，旧 fixtures 全绿。Verify: `uvx pytest tests/test_review_json_gate.py tests/test_specrail_schema.py -q`。
