...
- [ ] `SP188-T1` Owner: bundle-core | Depends on: approved spec, GH-165/GH-172 merged | Done when: closed manifest（含排序 `retired[]` 与置空自身字段后规范化计算的 `self_hash`）、路径/hash/稳定快照与 doctor 状态全覆盖；额外文件按 `files[] ∪ retired[]` 判定，`retired[]` 命中报 drift 而非不可迁移；doctor 的 `--repo . --target .` 只读自校验通过 | Verify: `python3 -m pytest -q tests/test_runtime_bundle.py -k "manifest or doctor or retired or self_hash or self_check"` | Covers: B-001 B-002 B-003 B-004 B-005 B-008 B-011 B-012 | 新增 lock/library/check CLI。
- [ ] `SP188-T2` Owner: installer | Depends on: SP188-T1 | Done when: 默认 dry-run、显式 apply、staging/atomic write/post-doctor 与取消失败全覆盖；apply 只写 `files[]` 与 manifest 自身、只删 target 上命中的 `retired[]`，其余路径写删均拒绝；source/target 重叠只在写路径 fail closed，升级用例的 post-doctor 达到 match | Verify: `python3 -m pytest -q tests/test_runtime_bundle.py -k "install or upgrade or overlap"` | Covers: B-006 B-007 B-008 B-011 | 新增 install CLI。
