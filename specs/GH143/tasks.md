...
- [ ] `SP143-T1` `checks/runtime_gate_rules.py` 新增 `_validate_tier_authorization()`：source 约定值 `tier_policy_gh143` 触发；`authorization_tier` 闭集校验；`standard_auto` 要求 `pr_tier` ∈ {fastlane, standard} + `pr_tier_evidence` + 非敏感 + 无争议 + 至少一项独立 tier 背书（gate 可校验的 CI tier-check artifact 引用，或 review artifact 中 reviewer-lane `tier_attestation: {pr_tier, attested: true, basis}` 且 tier 与 checkpoint 一致）；争议判定只信 review artifact（reviewer/merge-reviewer lane 或人工写入的 `tier_dispute: true`，以及 `tier_attestation.pr_tier` 与 checkpoint 不一致）；缺失/越界/无证据/无独立背书/争议全部 fail-closed 报 error。配套单元用例（含仅自报证据被拒、attestation tier 不一致视为争议、实现者侧 dispute 标记不作数）。Covers: B-002 B-003 B-004 B-006。Owner: agent. Done when: 校验函数覆盖全部拒绝分支且用例全绿. Verify: `python3 -m pytest -q tests/test_runtime_ledger_gate.py -k tier`
- [ ] `SP143-T2` `checks/runtime_ledger_gate.py` 接线：merge_authorization 校验块调用 T1 函数并传入顶层 `auth_mode`；standard_auto item 强制审计字段（`pr_tier`/`pr_tier_evidence`/`authorization_tier`）；`post_authorization_findings[]` 校验（critical 或非机械 finding 无 re_authorization → blocked；severity 缺失/越界按 critical；每条 finding 的 `severity`/`mechanical` 分类须对应 reviewer lane review artifact 的 `finding_classifications[]` 记录，仅实现者自报或两侧不一致按 critical 处理；mechanical 要求 `disposition: fixed_re_reviewed` 且复审证据对齐修复后 head）。Covers: B-001 B-005 B-006 B-008 B-009 B-010 B-012。Owner: agent. Done when: 全绿 fixture allowed、各拒绝 fixture blocked、既有用例零改动全绿. Verify: `python3 -m pytest -q tests/test_runtime_ledger_gate.py tests/test_runtime_ledger_review.py tests/test_runtime_ledger_queue.py`
...
- [ ] `SP143-T5` 端到端回归与审计演练:构造 standard_auto 全绿 checkpoint（含 reviewer `tier_attestation`）走 gate CLI 得 allowed；同一 fixture 逐一抹掉四类绿色证据、抹掉独立 tier 背书、改 heavy、置 `enforcement_sensitive: true`、注入 critical finding，逐个确认 blocked 及错误信息可定位。Covers: B-001 B-005 B-006 B-009 B-012。Owner: agent. Done when: 演练脚本化为测试或记录于 PR 描述附命令输出. Verify: `python3 checks/runtime_ledger_gate.py --checkpoint tests/fixtures/gh143-standard-auto.json --repo . --json`
- [ ] `SP143-T6` `schemas/review_result.schema.json` 扩展：新增可选字段 `tier_attestation`（`{pr_tier, attested, basis}`）、`finding_classifications[]`（`{finding_ref, severity, mechanical}`）、`tier_dispute`（bool），并在 schema 描述中注明仅 reviewer/merge-reviewer lane（或人工）写入；既有 review artifact 不带新字段时校验零变化。Covers: B-004 B-006 B-008 B-010。Owner: agent. Done when: schema 校验通过且既有 review artifact 样例零改动通过. Verify: `python3 checks/check_workflow.py --repo .`

## 并行拆分

T1 先行；T2 依赖 T1；T3 与 T1/T2 文件不相交可并行；T6 只改 schema 文件，可与 T1 并行但字段名以 T1 落定为准；T4 只改 skills/*/SKILL.md，依赖 T1-T3 确定的字段名后执行；T5 依赖 T2/T3/T6。
