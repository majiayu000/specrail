...
- [ ] `SP174-T3` Owner: integrity-integration | Depends on: SP174-T2 | Done when: queue 多文件集合进入 GH-172 normalized lock、installer（`tools/install_codex_skills.py`）和 installed doctor（`tools/check_installed_codex_skills.py` / `checks/installed_skill_integrity.py`，均已在 manifest 内），queue Startup 第一步执行 reference-graph + `--require-installed` doctor 双检查后才抓远端状态，缺失/漂移/越界/循环/取消均 fail closed | Verify: `python3 -m pytest -q tests/test_skill_reference_graph.py tests/test_install_codex_skills.py -k "reference or multifile"` | Covers: B-005 B-006 B-013 B-014 | 接入最新 GH-172 API，不复制 manifest/path/hash 逻辑。
