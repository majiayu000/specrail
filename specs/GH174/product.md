...
24. B-024 当 origin 为 `installed_copy` 时，startup doctor 必须只检查 resolver-attested
    ...
    manifest 枚举除自身外的完整 installed 文件闭集，但不得包含自身 digest 或完整
    source-lock digest。source lock 单向锁定 manifest bytes；current-invocation host
    attestation 必须同时绑定 manifest digest 与 source-lock digest，且 doctor receipt
    回显二者。这样生成图无环，同时 manifest、source-lock、实际文件任一错配仍 fail closed。
