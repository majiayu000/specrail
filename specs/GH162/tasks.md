...
- [ ] `SP162-T3` 同步 offline PR gate 和 runtime ledger 的 local-primary 断言及 fixtures。Covers: B-002 B-006 B-008 B-009 B-010 B-011。Owner: agent。Done when: hosted/缺失 provenance 双路径均 blocked；runtime 额外要求本地 machine-readable artifact，执行 schema + shared semantic validation，绑定 PR/lane/artifact ID/source/execution/head/completion/status/verdict/human gate/findings/prior findings；URL、缺失/非 JSON、hosted、legacy、字段错配、非法时间戳与 `clean` + findings 负例全部 blocked，语义错误仍保留 payload 供 binding/tier diagnostics；本地 independent 与合法 self-review 兼容路径通过。Verify: `python3 -m pytest -q tests/test_pr_gate_terminal.py tests/test_runtime_ledger_review.py tests/test_runtime_ledger_gate.py`。
...
- [ ] `SP162-T5` 运行全量测试、all-spec 与 diff 检查。Covers: B-001 B-002 B-003 B-004 B-005 B-006 B-007 B-008 B-009 B-010 B-011。Owner: agent。Done when: 所有 fresh commands 通过。Verify: `python3 -m pytest -q && python3 checks/check_workflow.py --repo . --all-specs && git diff --check`。
...
- Product invariants：B-001..B-011 全部被任务覆盖。
...
是 primary。Issue 创建、实现、review 与在所有门禁通过后的 merge 已获当前会话授权。
