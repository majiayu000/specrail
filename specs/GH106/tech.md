...
| auto 选择来源 | `skills/implx/SKILL.md:52-55`（queue skill 侧 `skills/specrail-implement-queue/SKILL.md:180-183`） | 持久化 `automation_policy.auth_mode` 是 review 安全基线；auto 仅由现时消息 `implx auto` 选择，不从配置提升 | B-008 |
| readiness 自动放行 | `skills/specrail-implement-queue/SKILL.md:71-77` | auto + complete/umbrella_covered → 补 label、记 `readiness_label_source: auto_drain`、报告列出；needs_spec/needs_tasks 永不放行 | B-001 B-002 |
| 弃用窗口默认值 | `skills/specrail-implement-queue/SKILL.md:133-138` | 用户未指定 → 下一 minor，`deprecation_default: true` 记入 checkpoint 与 PR 描述，移除仍受既有 gate | B-005 |
| 双 lane 失败例外 | `skills/specrail-implement-queue/SKILL.md:588-600` | auto + 两条不同独立 lane 失败 + `lane_failures[]` 完整 → implx auto 调用即 scoped 授权（actor: user、source: implx auto invocation）；单 lane 失败仍需重试 lane；review 模式不适用 | B-003 B-006 |
...
### 2. Readiness 自动放行（`specrail-implement-queue/SKILL.md:71-77`）
...
### 3. Post-failure self-review（`specrail-implement-queue/SKILL.md:588-600`）
...
### 5. 弃用窗口默认值（`specrail-implement-queue/SKILL.md:133-138`）
