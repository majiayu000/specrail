...
{"version":1,"issue":167,"complete":true,"paths":["checks/review_json_gate.py","checks/review_result_semantics.py","checks/pr_review_contract.py","checks/github_pr_evidence.py","checks/github_review_evidence.py","schemas/review_result.schema.json","schemas/pr_review_gate.schema.json","review/agent_first_review.md","skills/specrail-review-pr/SKILL.md","skills/specrail-implement-queue/SKILL.md","skills/implx/SKILL.md","integrations/threads.md","tests/test_review_json_gate.py","tests/test_review_result_semantics.py","tests/test_pr_gate_terminal.py","tests/test_github_pr_evidence.py","tests/test_github_pr_evidence_cli.py","tests/test_review_contract_docs.py","tests/test_specrail_schema.py"],"spec_refs":["specs/GH167/product.md","specs/GH167/tech.md","specs/GH167/tasks.md"]}
-->
...
| 文档与 skills | `review/agent_first_review.md`, `skills/specrail-review-pr/SKILL.md`, `skills/specrail-implement-queue/SKILL.md:322`, `skills/implx/SKILL.md`, `integrations/threads.md:194` | queue 权威合同仍写 full 最多 2 轮且人工可追加 full pass；threads 指向旧 rounds/modes 语义；carry-forward 未限制体积 | 四处运行合同必须同步停止条件、v2 输出与 maintainer escalation，并由测试防漂移 |
...
当前 `checks/review_json_gate.py` 为 693 行，不再承载 artifact-set/carry/authorization算法。新增共享逻辑进入 `checks/review_result_semantics.py`，CLI gate 只增加 bounded diff 的薄适配。实现后用 `wc -l` 断言两文件均小于 800 行；若 shared semantics 接近上限，再在同一实现 PR 中拆出专用模块并同步 planned paths，不允许超过硬上限后再补救。

### 7. 权威运行合同一致性

同步更新 `review/agent_first_review.md`、`skills/specrail-review-pr/SKILL.md`、权威 `skills/specrail-implement-queue/SKILL.md`、委派入口 `skills/implx/SKILL.md` 与 `integrations/threads.md`。queue/threads 不得继续允许第 2 轮 full 或以一次 `human_full_review_request` 无限追加；所有入口必须写明 cap=3、round>=2 scoped、cap 后停止并等待 exact-bound `continue_once`。新增 `tests/test_review_contract_docs.py` 检查稳定 marker 与禁止的旧短语，使任一文档回漂在 CI 中确定性失败。
...
| B-015 | review/queue/threads 合同 | 文档一致性测试要求四个入口都有 bounded marker，并拒绝旧 full-pass escape-hatch 文案 |
...
- 把新语义继续堆入 `review_json_gate.py`：被否，文件已有 693 行，且跨 artifact 逻辑本就属于 shared semantics。
...
- [ ] Contract docs：review/queue/implx/threads 稳定 marker 一致，旧 full-pass escape-hatch 文案被确定性测试拒绝。
