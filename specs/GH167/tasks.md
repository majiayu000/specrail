...
- [ ] `SP167-T6` 同步 reviewer 合同与编排：更新 `review/agent_first_review.md`、`skills/specrail-review-pr/SKILL.md`、`skills/implx/SKILL.md`，写明 v2 输出、第 2 轮起 exact scoped diff、compact carry、cap=3 后停止、人工逐轮 `continue_once` 与完整 finding 移交；明确 auto 模式和 `human_full_review_request` 均不能提供 cap 授权。Covers: B-002 B-003 B-004 B-005 B-006 B-010. Owner: agent. Depends on: SP167-T1..SP167-T5. Done when: 三处文档字段/停止语义一致且 workflow check 通过。Verify: `python3 checks/check_workflow.py --repo . --all-specs`.

- [ ] `SP167-T7` 端到端回归与 PR 证据演练：构造 v1 单轮兼容、v2 三轮、无授权第 4 轮、exact 授权第 4 轮、复用授权第 5 轮、full PR diff、compact carry 漏项/正文回放、当前 actionable finding 漏移交场景；逐层运行 review gate、manifest loader、GitHub adapter 和 PR gate。Covers: B-001..B-014. Owner: agent. Depends on: SP167-T1..SP167-T6. Done when: 所有正反场景有自动化测试，错误可定位且全量验证通过。Verify: `/usr/bin/python3 -m pytest -q && python3 checks/check_workflow.py --repo . --all-specs && git diff --check`.
