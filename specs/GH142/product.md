...
2. B-002 如果 `status: legacy` 字样只出现在 Linked Issue 小节之外（正文引用、示例、代码块），工具应不将其视为 legacy 声明，沿袭 GH130 B-005 的小节限定语义。
...
10. B-010 当为 spec 标记 legacy 时，只允许在 Linked Issue 小节追加显式文本行，不得删除或改写原 spec 其余内容；摘除 legacy 标记的唯一途径是重写该 spec 使其通过 `--gate` 默认阈值。
...
- [ ] 28 个老区段 spec 全部标记 legacy，标记集合可由枚举命令复现
...
| --- | --- |
| Empty / missing input | covered: B-003 B-007（未标记的浅 spec 视为第三态 FAIL；product.md 读取失败 fail-closed） |
