...
5. 导入两个测试模块与 support，确认关键全局名称仍绑定真实 production objects：
   ...
   assert general.validate_pack_assets is workflow.validate_pack_assets
   assert general.validate_auth_mode is workflow.validate_auth_mode
   assert paths.validate_spec_packet is workflow.validate_spec_packet
   assert paths.spec_packet_root is specrail_lib.spec_packet_root
