...
| Area | Files | Current behavior (anchors refreshed against the fixed tree) | Why relevant |
| --- | --- | --- | --- |
| Pack asset validation dispatch | `checks/check_workflow.py:171-186` | `validate_pack_assets` 从执行 checkout 自身路径（`Path(__file__).with_name`）加载可信 `pack_asset_validation.py`；缺失或无法加载时返回明确错误 | validation 逻辑已绑定到执行 checkout（B-001、B-002、B-006） |
| Spec root discovery | `checks/check_workflow.py:352-368`, `checks/check_workflow.py:381-386` | `discover_spec_packet_dirs` 对 configured root 缺失/非目录抛 `SpecRailError`；合法空目录返回空集合；非目录或重定向的 `GH<number>` 条目同样失败 | `--all-specs` fail-closed 语义（B-004、B-007） |
| Artifact path comparison | `checks/route_gate.py:122-130`, `checks/route_gate.py:343` | `required_artifact_path` 经 `spec_packet_artifact_paths` 取规范化路径；provided evidence 经 `validated_repo_relative_path` 规范化后比较 | required/provided 两侧同一规范形式（B-003） |
| Shared path contract | `checks/specrail_lib.py:262-278`, `checks/specrail_lib.py:324-338` | `validated_repo_relative_path` 拒绝反斜杠、绝对路径、盘符、`..` 与占位符；`spec_packet_artifact_paths` 是唯一 artifact path 解释器 | route gate 复用唯一解释器且约束不放宽（B-003、B-008） |
| Regression coverage | `tests/test_check_workflow.py:60`, `tests/test_check_workflow.py:262`, `tests/test_route_gate.py:134` | `test_trusted_pack_asset_validation_ignores_target_helper`、`test_main_validates_configured_spec_root`、`test_route_gate_accepts_normalized_configured_artifact_evidence` 覆盖三个审计负例 | 锁定合并后审计发现的行为 |
...
| B-004 missing/non-directory root fail closed | `checks/check_workflow.py` discovery | `python3 -m pytest -q tests/test_check_workflow.py -k validates_configured_spec_root` |
...
| B-006 trusted helper self-missing fail closed | `checks/check_workflow.py:171-186` | `python3 -m pytest -q tests/test_check_workflow.py -k trusted_pack_asset` |
| B-007 empty root vs broken root distinction | `checks/check_workflow.py:352-386` | `python3 -m pytest -q tests/test_check_workflow.py -k configured_root` |
| B-008 normalization keeps traversal constraints | `checks/specrail_lib.py:262-278` | `python3 -m pytest -q tests/test_route_gate.py -k normalized` |
