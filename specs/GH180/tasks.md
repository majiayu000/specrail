...
- [ ] `SP180-T3` 在 `checks/github_issue_evidence.py` 与 `schemas/issue_evidence.schema.json` 增加必填 UTC `collected_at`，并在 `checks/route_gate.py` 对 readiness route 强制 fresh trusted label evidence，拒绝 CLI self-report、缺失、未来或超窗 evidence；route 先验证 packet，再返回规范化 `issue_evidence_sha256`、逐 artifact sha256 与聚合 `packet_snapshot_sha256`，并提供 `--verify-result <saved-route.json>` consumer gate 重算当前 snapshot、校验 issue/route/evidence/freshness/decision。新增 `tests/test_issue_evidence_freshness.py`，更新 `tests/test_route_gate.py` 覆盖 collector、schema、错误 issue、freshness、摘要稳定性、saved result 与增删改 artifact 后旧结果失配。Covers: B-006 B-007 B-008 B-009 B-013 B-014 B-015 B-018 B-019。Owner: evidence gate lane。Depends on: SP180-T1。Done when: 只有 fresh trusted label evidence 可通过 readiness route；route JSON 绑定确切 issue/packet 内容，tasks 创建后必须重跑，consumer gate 可确定性拒绝旧摘要。Verify: `/usr/bin/python3 -m pytest -q tests/test_issue_evidence_freshness.py tests/test_route_gate.py`。
...
- Evidence gate lane 在 SP180-T1 后独占写入 `checks/github_issue_evidence.py`、`checks/route_gate.py`、`schemas/issue_evidence.schema.json`、`tests/test_issue_evidence_freshness.py`、`tests/test_route_gate.py`。
...
- Planned changes 恰好是 tech spec manifest 中的十四条路径；新增测试文件经 search-first 确认没有等价模块，且不修改已超过 800 行的 `tests/test_github_issue_evidence.py`。
...
- 实现阶段只能修改 tech manifest 中的十四条路径；四个 spec refs 不属于实现路径。
