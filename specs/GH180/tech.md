...
{"version":1,"issue":180,"complete":true,"paths":["AGENT_USAGE.md","README.md","checks/check_workflow.py","checks/github_issue_evidence.py","checks/route_gate.py","evaluate.py","examples/fixtures/issue-body-hint-ready-to-implement.json","examples/fixtures/issue-ready-to-implement.json","examples/fixtures/issue-ready-to-spec.json","examples/fixtures/issue-reserved-internal.json","schemas/issue_evidence.schema.json","skills-lock.json","skills/implx/SKILL.md","skills/specrail-implement-queue/SKILL.md","skills/specrail-implement/SKILL.md","skills/specrail-plan-tasks/SKILL.md","skills/specrail-workflow/SKILL.md","skills/specrail-write-product-spec/SKILL.md","skills/specrail-write-tech-spec/SKILL.md","templates/pull_request.md","templates/zh-CN/pull_request.md","tests/route_gate_test_support.py","tests/test_check_workflow.py","tests/test_check_workflow_paths.py","tests/test_configured_spec_path_review_regressions.py","tests/test_evaluate.py","tests/test_github_issue_evidence.py","tests/test_github_issue_route_evidence.py","tests/test_issue_evidence_freshness.py","tests/test_route_gate.py"],"spec_refs":["specs/GH180/bootstrap-evidence.json","specs/GH180/product.md","specs/GH180/tech.md","specs/GH180/tasks.md"]}
-->
...
- `route_gate --verify-result <saved-route.json> --evidence <fresh-issue-evidence.json>` 作为
  确定性 consumer gate：先对 fresh evidence 重做 issue/repository/source/trust/freshness 校验
  并重算 `issue_evidence_sha256`，再重算当前 packet 摘要，最后同时匹配 saved result 的
  issue、route、evidence/packet 摘要与 decision；不得把 saved hash 与 saved result 自身比较。
  queue 在生产代码 lane 前必须调用该模式。任何文件、evidence 或 freshness 状态改变都使旧结果
  失效，不允许把单独 validator success 与另一 snapshot 的 route success 拼接成授权。
...
5. 对 complete snapshot 重跑 implement route，并立即用
   `--verify-result <result> --evidence <fresh-evidence>` 对当前文件和 fresh issue snapshot 验证结果；
...
来自 fresh trusted GitHub evidence。staged 写作完成后必须等待或在 auto 合同下真实设置 readiness
并重新采集；tasks 完成后必须重跑 route 与 consumer gate。七个 skill 修改后只更新其
...
evidence 与 packet 摘要、decision、missing/reasons。消费者必须用
`--verify-result <result> --evidence <fresh-evidence>` 重算两类摘要匹配，不得拼接不同
snapshot 的成功结果或把 saved hash 自比较后声称 implementation-ready。
...
| B-007 B-013 | route gate、plan-tasks、direct implement、implx 与 queue coverage | staged=`needs_tasks`；tasks 后以 fresh evidence 重跑并匹配 complete snapshot 才能进入生产实现 |
| B-008 B-009 | issue evidence schema/collector、route gate、既有 fixtures/tests | CLI state、body hint、过期/未来 evidence、错误 issue/冲突 label 全部 fail closed；fresh trusted label 才通过，旧允许断言改为拒绝断言 |
...
| B-014 B-015 | validator/route snapshot hashes + `--verify-result` + fresh evidence | 相同输入摘要一致；label/evidence/artifact 任一漂移后旧 result 被确定性拒绝 |
...
coverage 重跑，再用 `--verify-result` 与 fresh issue evidence 同时匹配 complete snapshot；
生产实现只消费 fresh 且两类摘要均匹配的结果。
