...
- `workflow.yaml` 不在 planned changes 中，现有 auth policy 保持原样：persisted/default
  ...
  auto 路径则要求 `--auth-mode auto --auth-evidence <invocation-authorization.json>` 指向独立、
  闭合的 runtime authorization record，至少含 `invocation_id`、
  `auth_mode: auto`、issue、route、授权来源、`recorded_at`、精确
  `waived_human_gates: ["spec_approval"]` 与 invocation scope。`--auth-mode auto`、持久化配置或
  readiness label 单独都不能构成 waiver；record 缺失、跨 invocation/issue/route 复用或字段冲突
  必须阻断。route result 绑定 `spec_approval_authorization_sha256` 和 authorization kind；
...
- `route_gate --verify-result <saved-route.json> --evidence <fresh-issue-evidence.json>
  ...
  ordered lifecycle 或 invocation-scoped waiver，把 fresh duplicate evidence 交给 duplicate
  gate 重做 freshness、open PR 与 branch 检查，然后重算当前 packet 摘要，最后匹配 saved
  result 的 issue、route、authorization/issue/duplicate/packet 摘要与 allowed decision。
  不得把 saved hash 与 saved result 自身比较。新 invocation、PR/branch、文件、label/lifecycle
  或 freshness 状态变化都使旧结果失效。
...
2. review/default 路径采集有权限 actor 的有序
   ...
   则记录仅限该 invocation/issue/route 的 `spec_approval` waiver；
...
5. 对 complete snapshot 重跑 implement route，并立即用
   ...
   --duplicate-evidence <fresh-duplicate-evidence>` 对当前文件、fresh issue、
...
- 主 agent 记录本次 invocation-scoped `auth_mode: auto` waiver，maintainer 把 live label 直接从
  `ready_to_spec` 切到 `ready_to_implement`；该 transition 没有经过正常的
  `spec_pr_open → spec_review → spec_approved`，不得描述为 review lifecycle；该 waiver
  只覆盖当次 `spec_approval`，不证明 readiness、duplicate 或 saved-result gate；
...
的 lifecycle evidence，在明确 auto invocation 中来自 invocation-scoped waiver record。
...
| B-008 B-009 | issue/lifecycle evidence schema/collector、route gate auth context、label catalog、既有 fixtures/tests | review/default implement 要 ordered human lifecycle approval；明确 auto invocation 可凭 scoped record waive spec_approval，但不能 waive fresh trusted readiness/duplicate/snapshot binding；CLI state/readiness label、无 record 的 auto 与过期/错误 evidence 全部 fail closed |
...
| B-014 B-015 | validator/route authorization/evidence/snapshot hashes + `--verify-result` + fresh issue/duplicate evidence | 相同输入摘要一致；auth mode、invocation/lifecycle、label、duplicate 或 artifact 任一漂移及新 PR/branch 出现后旧 result 被确定性拒绝 |
...
lifecycle 或 invocation-scoped auto waiver、duplicate-work evidence → route gate 校验
authorization kind、freshness、可信 label、open PR/branch snapshot 和 packet 内容 → 带
authorization/issue/duplicate/packet 摘要的 decision JSON。
...
- Security: shape 与 readiness label 都不得被当作 spec approval；review implement 验证有权限
  人类 lifecycle approval，auto implement 验证 current invocation-scoped waiver；两者都验证
  可信 readiness、duplicate、packet 与 consumer evidence。
- Compatibility: CLI 新增确定性 shape 行但保留既有最终消息与 exit code；旧完整 packet 无需改写。
- Race: route JSON 显式绑定 auth mode、approval-or-waiver、issue/duplicate evidence 与 packet
  内容摘要；生产操作前必须重采并重算匹配，不得跨 invocation 复用 waiver、旧 shape/readiness
...
- [ ] Evidence/route: `/usr/bin/python3 -m pytest -q tests/test_issue_evidence_freshness.py
  ...
  覆盖 review ordered lifecycle/human permission、auto invocation-scoped waiver 正负例、
  CLI state/readiness label self-report、缺失/未来/超窗 issue/duplicate evidence、错误 issue、
  新 PR/branch、authorization/内容摘要与 artifact 漂移。
