...
| current invocation trust | search-first 未发现现有 route trust-anchor verifier；runtime checkpoint/tier authorization 只覆盖队列/merge，不证明 route caller 的 current invocation | authorization record 可自报 `invocation_id`，旧同 issue/route record 与旧 saved result 可跨 invocation 重放 | 新增窄化的 signed runtime context verifier/schema；trust root/current generation 必须来自 runtime-owned provider，不接受 CLI、record 或 saved result 提供 |
...
- `workflow.yaml` 不在 planned changes 中，现有 auth policy 保持原样：persisted/default
  ...
  auto 路径则要求 `--auth-mode auto --auth-evidence <invocation-authorization.json>
  --runtime-invocation-context <signed-current-context.json>`。authorization record 至少含
  `invocation_id`、
...
- `route_gate --verify-result <saved-route.json> --evidence <fresh-issue-evidence.json>
  ...
  还必须追加 `--auth-mode auto --auth-evidence <same-invocation-authorization.json>
  --runtime-invocation-context <same-live-current-context.json>`。consumer
  先对 fresh issue
...
5. 对 complete snapshot 重跑 implement route，并立即用
   ...
   --duplicate-evidence <fresh-duplicate-evidence>`（auto 同时重供 authorization record 和
   同一 live current-invocation context）对当前文件、fresh issue、
...
`--verify-result <result> --evidence <fresh-evidence>
--duplicate-evidence <fresh-duplicate-evidence>`（auto 还必须重供 authorization record 与
live current-invocation context）重算所有摘要并重跑 duplicate gate，不得拼接
...
| B-008 B-009 | issue/lifecycle evidence schema/collector、runtime invocation verifier/schema、route gate auth context、label catalog、既有 fixtures/tests | review/default implement 要 ordered human lifecycle approval；auto 必须同时验证 scoped record 与 runtime-owned live current-invocation anchor，且只能 waive spec_approval；CLI/caller/record/saved-result 自报 current、无 anchor、伪造/过期/旧 generation context 与过期/错误 evidence 全部 fail closed |
...
| B-014 B-015 | validator/route authorization/runtime-context/evidence/snapshot hashes + `--verify-result` + fresh issue/duplicate evidence | 相同输入摘要一致；route 与 consumer 各自查询 runtime current generation；auth mode、record/context、invocation/lifecycle、label、duplicate 或 artifact 任一漂移及新 PR/branch 出现后旧 result 被确定性拒绝 |
...
- Security: shape 与 readiness label 都不得被当作 spec approval；review implement 验证有权限
  ...
  current-invocation anchor；两者都验证
...
- [ ] Evidence/route: `/usr/bin/python3 -m pytest -q tests/test_issue_evidence_freshness.py
  tests/test_route_gate.py tests/test_runtime_invocation_context.py tests/test_duplicate_work_gate.py
  tests/test_github_duplicate_evidence.py`，覆盖 review ordered lifecycle/human permission、
  auto authorization-record + runtime current-invocation anchor 正负例、
  CLI state/readiness label self-report、缺失/未来/超窗 issue/duplicate evidence、错误 issue、
  新 PR/branch、缺/伪造/过期/旧 generation context、旧 record 与旧 saved result 跨 invocation
  重放、authorization/context/内容摘要与 artifact 漂移。
