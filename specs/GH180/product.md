...
8. B-008 除 B-016 明确限定的 GH-180 一次性旧-validator bootstrap 外，从 spec 写作
   ...
   还是 `complete`，均不得以文件形态跳过状态转换。B-016 不证明这条正常链已发生，
   也不能授权任何其它 packet 跳过它。
9. B-009 readiness-sensitive 判定只能消费可信、在配置 freshness 窗口内、与 linked
   issue 一致的 label evidence；CLI `--state`、缺失采集时间、未来时间、body hint、agent
...
14. B-014 readiness-sensitive 结果必须同时绑定 issue evidence 的规范化内容摘要与
    当前 packet 中已发现 artifact 的路径、内容 sha256 和聚合 snapshot sha256。消费保存的
    route 结果时，consumer gate 必须接收重新采集的 fresh issue evidence 或自己重新采集，
    不得只把 saved evidence hash 与 saved result 自身比较。任一 artifact 被增加、删除或修改，
    linked issue evidence 变化或 freshness 超限后，消费者必须拒绝旧结果并基于最新 snapshot
    重跑；旧结果不得授权新内容。
...
- [ ] readiness 缺失、冲突、超出 freshness 窗口、未来时间、错误 issue、body hint、CLI
  自报状态与 artifact 并发漂移均不能产生 implementation-ready 结论；route 结果携带并
  绑定 issue evidence 摘要、artifact 内容哈希与 packet snapshot 摘要，consumer gate
  使用 fresh issue evidence 重验，而不是对 saved hash 做自比较。
...
| 授权/权限 | covered: B-003, B-008, B-009, B-016, B-017（shape 不授权；正常 route 只接受可信 evidence；GH-180 exception 不扩权） |
...
| 证据与审计完整性 | covered: B-009, B-010, B-014..B-018（fresh evidence、内容摘要、snapshot、unproven bootstrap 字段与判定理由均可追溯） |
