...
14. B-014 readiness-sensitive 结果必须同时绑定 issue evidence 的规范化内容摘要与
    当前 packet 中已发现 artifact 的路径、内容 sha256 和聚合 snapshot sha256。任一
    artifact 被增加、删除或修改，linked issue evidence 变化或 freshness 超限后，消费者
    必须拒绝旧结果并基于最新 snapshot 重跑；旧结果不得授权新内容。
...
16. B-016 GH-180 在旧 validator 下的 bootstrap 必须分两阶段并把真实证据持久化到本
    packet 的 tracked `bootstrap-evidence.json`：先以
...
17. B-017 B-016 的 bootstrap 必须限于 GH-180 本次 lifecycle-contract 迁移并被显式
    记录；tracked evidence 缺失、采集失败、权限不足、issue/head/packet 不匹配、未完成真实状态
...
- [ ] readiness 缺失、冲突、超出 freshness 窗口、未来时间、错误 issue、body hint、CLI
  ...
  绑定 issue evidence 摘要、artifact 内容哈希与 packet snapshot 摘要。
- [ ] 既有完整 packet 保持兼容；提前生成 tasks 的 `ready_to_spec` 在途 PR 可以在
  可信 route evidence 下删除 tasks 并恢复为 `staged`。
- [ ] GH-180 bootstrap 的 tracked evidence 证明先接受 live `ready_to_spec` +
  `write_spec: allowed` 形成
...
| 证据与审计完整性 | covered: B-009, B-010, B-014..B-018（采集时间、来源、内容摘要、snapshot、重试、tracked bootstrap 与判定理由均可追溯） |
