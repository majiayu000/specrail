...
8. B-008 从 spec 写作进入实现时，默认及 `auth_mode: review` 路径必须同时具备：可信、
   ...
   trust root 校验；record 中的 `invocation_id` 只能参与匹配，不能自行证明“当前”。
   持久化配置、packet shape、readiness label、旧 authorization record、旧 anchor 或旧 saved
   result 均不能构成 waiver。无论是否 waive `spec_approval`，fresh trusted readiness、fresh
...
14. B-014 readiness-sensitive 结果必须同时绑定 issue evidence、spec approval authorization
    ...
    同一个仍 live、仍为 runtime current 的 trusted invocation anchor，并重验其签名、generation、
    expiry、issue/route、record 摘要和精确 waived gates。consumer 随后重验 freshness、open PR
