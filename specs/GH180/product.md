...
8. B-008 从 spec 写作进入实现时，默认及 `auth_mode: review` 路径必须同时具备：可信、
   ...
   现有 policy waive `spec_approval`；该 waiver 必须记录 invocation identity、issue/route
   scope、授权来源、时间与被 waive 的 gate，不能由持久化配置、packet shape 或 readiness
   label 推断。无论是否 waive `spec_approval`，fresh trusted readiness、fresh
...
14. B-014 readiness-sensitive 结果必须同时绑定 issue evidence、spec approval authorization
    ...
    auth mode/authorization record、freshness、open PR 与 remote branch snapshot 后再比较摘要；
    不得只把 saved hash 与 saved result 自身比较。任一 artifact、linked issue、approval/waiver、
    duplicate-work snapshot 或 freshness 状态变化后，消费者必须拒绝旧结果并基于最新 snapshot
    重跑；旧结果不得授权新内容。
...
- [ ] `write_spec` 在 `ready_to_spec` 只形成 product/tech；`implement` 在 review 路径凭
  ...
  `spec_approval` waiver 入场；两者都仍须 fresh trusted `ready_to_implement`、fresh
  duplicate-work evidence 与有效 product/tech，再创建并验证 tasks，最后才允许代码实现。
- [ ] readiness/lifecycle/auto-waiver 缺失或冲突、超出 freshness 窗口、未来时间、错误 issue、
  ...
  均不能产生 implementation-ready 结论；route 结果绑定 issue、approval-or-waiver、
  duplicate-work 与 packet 摘要，consumer gate 使用同一 invocation authorization、fresh issue
  和 fresh duplicate-work evidence 重验，而不是对 saved hash 做自比较。
...
| 授权/权限 | covered: B-003, B-008, B-009, B-016, B-017（shape/readiness label 不单独授权；review implement 验证人类 lifecycle approval，明确 auto invocation 只 waive spec_approval；两者都验证其余 current evidence） |
| 并发/竞态 | covered: B-014（artifact、issue、lifecycle/waiver 或 duplicate-work snapshot 漂移后必须重判） |
...
`collected_at`/hash 明确为 `unproven`；它只记录当次 invocation 对 `spec_approval` 的 waiver，
不是正常生命周期、standing authorization 或其它 gate 的 waiver。其它在途 spec PR 必须使用
