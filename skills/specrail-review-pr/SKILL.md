...
- Round 1 is the only full review. Collect all findings before any repair.
  Round 2 is the only default re-review and must use `review_mode:
  resumed | diff_only`; `base_head_sha` must equal the previous
...
- Before every `review_round >= 3`, stop and route the PR to `human_decisions`.
  Continue exactly once only with an
