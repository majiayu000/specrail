...
5. 编辑后比较 normalized nodes、36 个顶层函数完整 AST、production symbol identity、
   两个既有 skip/xfail 调用与 focused outcome parity、精确三路径 allowlist和逐文件
   行数；任一差异都阻断交付。
...
1. 记录实际 base，创建固定 `Python 3.13.11` + `pytest==9.1.1` 的临时环境，
   并在后续步骤中只复用该环境的同一个 Python 解释器：

   ```sh
   git rev-parse HEAD > /tmp/gh120-impl-base-sha.txt
   command -v uv > /tmp/gh120-uv-bin.txt
   uv_bin=$(cat /tmp/gh120-uv-bin.txt)
   "$uv_bin" venv --clear --python '3.13.11' /tmp/gh120-pytest-env
   "$uv_bin" pip install --python /tmp/gh120-pytest-env/bin/python 'pytest==9.1.1'
   printf '%s\n' /tmp/gh120-pytest-env/bin/python > /tmp/gh120-python-bin.txt
   python_bin=$(cat /tmp/gh120-python-bin.txt)
...
2. 保存编辑前 normalized nodes、全库 count 与 focused outcome counts：
   ...
   ```sh
   python_bin=$(cat /tmp/gh120-python-bin.txt)
   if ! "$python_bin" -m pytest --collect-only -q tests/test_route_gate.py \
     > /tmp/gh120-before-focused-collect.txt 2>&1; then
     tail -n 20 /tmp/gh120-before-focused-collect.txt
     exit 1
   fi
   sed -n '/::/s/^[^:]*:://p' /tmp/gh120-before-focused-collect.txt \
     | LC_ALL=C sort > /tmp/gh120-before-nodes.txt
   if ! "$python_bin" -m pytest --collect-only -q \
     > /tmp/gh120-before-all-collect.txt 2>&1; then
     tail -n 20 /tmp/gh120-before-all-collect.txt
     exit 1
   fi
   awk '/::/{count++} END{print count+0}' /tmp/gh120-before-all-collect.txt \
     > /tmp/gh120-before-all-count.txt
   ...
   if ! "$python_bin" -m pytest -q -r a tests/test_route_gate.py \
     > /tmp/gh120-before-focused-pytest.txt 2>&1; then
     tail -n 20 /tmp/gh120-before-focused-pytest.txt
     exit 1
   fi
   rg -o '[0-9]+ (passed|failed|errors?|skipped|xfailed|xpassed)' \
     /tmp/gh120-before-focused-pytest.txt | LC_ALL=C sort \
     > /tmp/gh120-before-focused-outcomes.txt
   test -s /tmp/gh120-before-focused-outcomes.txt
   ```
...
3. 拆分后保存同形证据并比较：
   ...
   python_bin=$(cat /tmp/gh120-python-bin.txt)
   if ! "$python_bin" -m pytest --collect-only -q tests/test_route_gate*.py \
     > /tmp/gh120-after-focused-collect.txt 2>&1; then
     tail -n 20 /tmp/gh120-after-focused-collect.txt
     exit 1
   fi
   sed -n '/::/s/^[^:]*:://p' /tmp/gh120-after-focused-collect.txt \
     | LC_ALL=C sort > /tmp/gh120-after-nodes.txt
   if ! "$python_bin" -m pytest --collect-only -q \
     > /tmp/gh120-after-all-collect.txt 2>&1; then
     tail -n 20 /tmp/gh120-after-all-collect.txt
     exit 1
   fi
   awk '/::/{count++} END{print count+0}' /tmp/gh120-after-all-collect.txt \
     > /tmp/gh120-after-all-count.txt
...
4. 比较全部顶层函数 AST，并拒绝重复函数、support 测试、`pytestmark` 以及
   ...
   assert len(before_skip_xfail) == 2, len(before_skip_xfail)
...
7. 执行 focused/full 与 workflow 验证：
   ...
   python_bin=$(cat /tmp/gh120-python-bin.txt)
   if ! "$python_bin" -m pytest -q -r a tests/test_route_gate*.py \
     > /tmp/gh120-focused-pytest.txt 2>&1; then
     tail -n 20 /tmp/gh120-focused-pytest.txt
     exit 1
   fi
   tail -n 20 /tmp/gh120-focused-pytest.txt
   rg -o '[0-9]+ (passed|failed|errors?|skipped|xfailed|xpassed)' \
     /tmp/gh120-focused-pytest.txt | LC_ALL=C sort \
     > /tmp/gh120-after-focused-outcomes.txt
   diff -u /tmp/gh120-before-focused-outcomes.txt \
     /tmp/gh120-after-focused-outcomes.txt
...
| B-004 | 全部迁移函数与 production bindings | 步骤 4-5、7；36-function AST、2-call skip/xfail AST、identity 与 focused outcome parity 通过 |
