    ...
    fake_gh.write_text(
        "\n".join(
            [
                ...
                f"files_payload = {json.dumps(files_payload())!r}",
                f"rest_files = {json.dumps(rest_files_payload())!r}",
                "args = sys.argv[1:]",
                "if args[:2] == ['pr', 'view']:",
                "    print(pr_payload)",
                "elif args[:2] == ['api', 'graphql']:",
                "    # A review manifest may carry a tier attestation, so the",
                "    # collector now also requests the PR file snapshot.",
                "    if any('SpecRailPRFiles' in a for a in args):",
                "        print(files_payload)",
                "    else:",
                "        print(threads_payload)",
                "elif args[:2] == ['api', '--method'] and args[2] == 'GET':",
                "    print(rest_files)",
                "else:",
