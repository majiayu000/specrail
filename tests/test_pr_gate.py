...
def test_pr_gate_tier_metadata_never_replaces_human_authorization() -> None:
    result = evaluate_pr_gate(_tier_evidence())

    assert result["decision"] == "needs_human"
    assert "human_authorization" in result["missing"]
    assert not any("tier authorization" in item for item in result["satisfied"])


def test_pr_gate_tier_metadata_with_human_authorization_allowed() -> None:
